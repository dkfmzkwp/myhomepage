...
if(isset($postAndget_mode)&&$postAndget_mode=="insert"){
  ...
  $copied_file_name = "";
    ...
    $sql="INSERT INTO `wod` VALUES (null,'$q_userid','$q_subject','$q_content','$regist_day',0,'$category','$upfile_name','$copied_file_name','$upfile_type');";
    ...
    $sql="SELECT num from `wod` where id ='$userid' order by num desc limit 1;";
    $result = mysqli_query($con,$sql);
    if (!$result) {
    ...
    mysqli_close($con);
        ...
        $sql ="DELETE FROM `wod` WHERE num=$q_num";
        $result = mysqli_query($con,$sql);
        if (!$result) {
          die('Error: ' . mysqli_error($con));
        }

        $sql ="DELETE FROM `code_ripple` WHERE parent=$q_num";
        $result = mysqli_query($con,$sql);
        if (!$result) {
          die('Error: ' . mysqli_error($con));
        }
        ...
        echo "<script>location.href='./code_list.php';</script>";
    ...
    $regist_day=date("Y-m-d (H:i)");
      ...
      if(isset($_POST['del_file']) && $_POST['del_file'] =='1'){
        ...
        $sql="SELECT `file_copied_0` from `wod` where num ='$postAndget_num';";
        $result = mysqli_query($con,$sql);
        if (!$result) {
          alert_back('Error: 6' . mysqli_error($con));
          // die('Error: ' . mysqli_error($con));
        ...
        $sql="UPDATE `wod` SET `file_name_0`='', `file_copied_0` ='', `file_type_0` =''  WHERE `num`=$postAndget_num;";
        $result = mysqli_query($con,$sql);
        if (!$result) {
          die('Error: ' . mysqli_error($con));
        }
      ...
      if(!empty($_FILES['upfile']['name'])){
        ...
        $sql="UPDATE `wod` SET `file_name_0`= '$upfile_name', `file_copied_0` ='$copied_file_name', `file_type_0` ='$type[0]' WHERE `num`=$postAndget_num;";
        $result = mysqli_query($con,$sql);
        if (!$result) {
          die('Error: ' . mysqli_error($con));
        }
      ...
      $sql="UPDATE `wod` SET `subject`='$q_subject',`content`='$q_content',`regist_day`='$regist_day'  WHERE `num`=$postAndget_num;";
      $result = mysqli_query($con,$sql);
      if (!$result) {
        die('Error: ' . mysqli_error($con));
      }

      echo "<script>location.href='./view.php?num=$postAndget_num&page=1&hit=$hit';</script>";
